...
void treset(void) {
    ...
    for (i = tabspaces; i < (uint)term.col; i += tabspaces) term.tabs[i] = 1;
    term.top  = 0;
    term.bot  = term.row - 1;
...
void tinsertblankline(int n) {
    if (BETWEEN(term.c.y, term.top, term.bot)) tscrolldown(term.c.y, n, 0);
}
...
void tputtab(int n) {
    ...
    if (n > 0) {
        while (x < (uint)term.col && n--)
            for (++x; x < (uint)term.col && !term.tabs[x]; ++x) /* nothing */
                ;
    } else if (n < 0) {
        while (x > 0 && n++)
    ...
    term.c.x = LIMIT(x, 0U, term.col - 1U);
}
